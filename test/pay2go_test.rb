...
class TestPay2go < MiniTest::Test
  def setup
  ...
  def test_raise_invalid_mode_error
    assert_raises Pay2go::InvalidMode do
      Pay2go::Client.new(mode: :invalid)
    end
  end

  def test_raise_missing_option_error
    assert_raises Pay2go::MissingOption do
      Pay2go::Client.new
    end
  end

  def test_raise_missing_parameter_error
    assert_raises Pay2go::MissingParameter do
      @client.generate_mpg_params
    end
  end

  def test_raise_unsupported_type_error
    assert_raises Pay2go::UnsupportedType do
